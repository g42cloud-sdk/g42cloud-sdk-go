...
func TestEnvCredentialProvider_GetCredentials3(t *testing.T) {
	...
	p = BasicCredentialEnvProvider()
	assert.Nil(t, err)
	err = os.Setenv("G42CLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err = p.GetCredentials()
	assert.Nil(t, err)
...
func TestEnvCredentialProvider_GetCredentials5(t *testing.T) {
	...
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	err = os.Setenv("G42CLOUD_SDK_IDP_ID", MockIdpId)
	assert.Nil(t, err)
	err = os.Setenv("G42CLOUD_SDK_ID_TOKEN_FILE", MockIdTokenFile)
	assert.Nil(t, err)
	err = os.Setenv("G42CLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err := p.GetCredentials()
...
func resetEnv() error {
	envs := []string{
		"G42CLOUD_SDK_AK",
		"G42CLOUD_SDK_SK",
		"G42CLOUD_SDK_PROJECT_ID",
		"G42CLOUD_SDK_DOMAIN_ID",
		"G42CLOUD_SDK_CREDENTIAL_TYPE",
		"G42CLOUD_SDK_IDP_ID",
		"G42CLOUD_SDK_ID_TOKEN_FILE",
	}
